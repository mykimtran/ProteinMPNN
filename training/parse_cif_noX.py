...
aa2idx = {(r, a): i for r, atoms in zip(RES_NAMES, ATOM_NAMES) for i, a in enumerate(atoms)}
    ...
    for i in range(L):
        for j, xyz_ij in enumerate(xyz[i]):
            ...
            f.write(
                ...
                % ("ATOM", ctr, a, r, "A", i + 1, xyz_ij[0], xyz_ij[1], xyz_ij[2], 1.0, bfac[i, j])
            )
            if a == "CA":
                idx.append(i)
            ctr += 1
...
def TMalign(chainA, chainB):
    ...
    mtx.seek(0)
    tu = np.fromstring("".join(l[2:] for l in mtx.readlines()[2:5]), dtype=float, sep=" ").reshape((3, 4))
    t = tu[:, 0]
    u = tu[:, 1:]
    ...
    alnAB = np.stack((idxA[(np.cumsum(ss1) - 1)[mask]], idxB[(np.cumsum(ss2) - 1)[mask]]))
...
def parseAssemblies(data, chids):
    ...
    xforms = {"asmb_chains": None, "asmb_details": None, "asmb_method": None, "asmb_ids": None}
    ...
    for index in range(assembly_gen.getRowCount()):
        ...
        oper_list = [
            parseOperationExpression(expression) for expression in re.split("\(|\)", oper_expression) if expression
        ]
        ...
        if len(oper_list) == 1:
            ...
            xform = np.stack([opers[o1] @ opers[o2] for o1 in oper_list[0] for o2 in oper_list[1]])
...
def parse_mmcif(filename):
    ...
    pdb2asym = dict(
        {
            (r[pdbx_poly_seq_scheme.getIndex("pdb_strand_id")], r[pdbx_poly_seq_scheme.getIndex("asym_id")])
    ...
    num2seq = {
        r[entity_poly.getIndex("entity_id")]: r[entity_poly.getIndex("pdbx_seq_one_letter_code_can")].replace("\n", "")
        for r in entity_poly.getRowList()
        if r[entity_poly.getIndex("type")] == "polypeptide(L)"
    }
...
for k, v in chains.items():
    ...
    print(
        ">%s_%s %s %s %s %d %d\n%s"
        % (ID, k, metadata["date"], metadata["method"], metadata["resolution"], len(v["seq"]), nres, v["seq"])
    )

    torch.save({kc: torch.Tensor(vc) if kc != "seq" else str(vc) for kc, vc in v.items()}, f"{OUT}_{k}.pt")
